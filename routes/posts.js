...
router.post("/", async function (req, res, next) {
  try {
    ...
    const result = await db.query(
      `INSERT INTO posts (title, description, body, category) 
        VALUES ($1, $2, $3, $4) 
...
router.put("/:id", async function (req, res, next) {
  try {
    const {title, body, description, category} = req.body;
    const result = await db.query(
      `UPDATE posts SET title=$1, description=$2, body=$3, category=$5
        WHERE id = $4 
        RETURNING id, title, description, body, votes, category`,
      [title, description, body, req.params.id, category]);
    return res.json(result.rows[0]);
  } catch (e) {
    return next(e);
  }
